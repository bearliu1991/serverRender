<template>
  <div class="cs-adress" :class="terminal">
    <header>
      ...
      <List 
        :terminal="terminal"
        :adressList="adressList"   :deleteAddress="deleteAddress"
      ...
      <div class="adress_form" v-if="formShow">
        <div class="container" :class="formTopAni ? 'container_transform': ''">
          ...
          <div class="default-adress" @click="defaultAdressChoose">
            <div class="checks " :class="formData.isDefault ? 'el-icon-check': ''"></div>
...
export default {
  ...
  methods: {
    ...
    async newOrUpdateAddress(){
      await (this.addOrEditAddress ? this.$api.address.saveAddress({...this.$refs.formData.formData, isDefault: this.formData.isDefault}):  this.$api.address.updateAddress({...this.$refs.formData.formData, isDefault: this.formData.isDefault}));
      this.getAddressList();
      this.formShowDeal();
      this.$alert({
        text: '修改成功！',
