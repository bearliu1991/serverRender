      ...
      <div class="adress_form" v-if="formShow">
        <div class="container" :class="formTopAni ? 'container_transform': ''">
          ...
          <div class="default-adress" @click="defaultAdressChoose">
            <div class="checks " :class="defaultAdress ? 'el-icon-check': ''"></div>
...
export default {
  ...
  methods: {
    ...
    async newOrUpdateAddress(){
      const data = await (this.addOrEditAddress ? this.$api.address.saveAddress({...this.$refs.formData.formData}):  this.$api.address.updateAddress({...this.$refs.formData.formData}));
      if(data === 1){
        this.getAddressList();
        this.formShowDeal();
        this.$alert({
